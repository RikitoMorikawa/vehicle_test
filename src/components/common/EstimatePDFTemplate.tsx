  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          fontFamily: "system-ui, -apple-system, sans-serif",
          pageBreakAfter: documentType === "order" ? "always" : "auto",
        }}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <table className="w-full border-2 border-t-0 border-black text-xs">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="p-2">車両本体価格</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.base_price)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">値引き</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.discount)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">車検整備費用</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.inspection_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">付属品・特別仕様 ・・・(A)</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.accessories_fee)}</td>
                  </tr>
                  <tr className="border-b border-black bg-gray-100">
                    <td className="p-2 font-bold">車両販売価格①</td>
                    <td className="p-2 text-right font-bold">¥{formatNumber(data.salesPrices.vehicle_price)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">税金・保険料 ・・・(B)</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.tax_insurance)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">預り法定費用 ・・・(C)</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.legal_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">手続代行費用 ・・・(D)</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.processing_fee)}</td>
                  </tr>
                  <tr className="border-b border-black bg-gray-100">
                    <td className="p-2 font-bold">販売諸費用②</td>
                    <td className="p-2 text-right font-bold">¥{formatNumber(data.salesPrices.misc_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">現金販売価格①＋②</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.total_price)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">内消費税</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.consumption_tax)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">下取車価格</td>
                    <td className="p-2 text-right">¥{formatNumber(data.salesPrices.trade_in_price)}</td>
                  </tr>
                  <tr className="border-b border-black">
            ...
            <div>
              ...
              <table className="w-full border-2 border-t-0 border-black text-xs">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="p-2">車名</td>
                    <td className="p-2">{data.tradeInVehicle?.vehicle_name || ""}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">登録番号</td>
                    <td className="p-2">{data.tradeInVehicle?.registration_number || ""}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">走行距離</td>
                    <td className="p-2">{data.tradeInVehicle?.mileage ? formatNumber(data.tradeInVehicle.mileage) + "km" : ""}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">初度登録年月</td>
                    <td className="p-2">{data.tradeInVehicle?.first_registration_date ? formatDate(data.tradeInVehicle.first_registration_date) : ""}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">車検満了日</td>
                    <td className="p-2">{data.tradeInVehicle?.inspection_expiry_date ? formatDate(data.tradeInVehicle.inspection_expiry_date) : ""}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">車台番号</td>
                    <td className="p-2">{data.tradeInVehicle?.chassis_number || ""}</td>
                  </tr>
                  <tr>
                    <td className="p-2">外装色</td>
                    <td className="p-2">{data.tradeInVehicle?.exterior_color || ""}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <table className="w-full border-2 border-t-0 border-black text-xs">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="p-2">自動車税</td>
                    <td className="p-2 text-right">¥{formatNumber(data.taxInsuranceFees.automobile_tax)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">環境性能割</td>
                    <td className="p-2 text-right">¥{formatNumber(data.taxInsuranceFees.environmental_performance_tax)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">重量税</td>
                    <td className="p-2 text-right">¥{formatNumber(data.taxInsuranceFees.weight_tax)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">自賠責保険料</td>
                    <td className="p-2 text-right">¥{formatNumber(data.taxInsuranceFees.liability_insurance_fee)}</td>
                  </tr>
                  <tr>
                    <td className="p-2">任意保険料</td>
                    <td className="p-2 text-right">¥{formatNumber(data.taxInsuranceFees.voluntary_insurance_fee)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div>
              ...
              <table className="w-full border-2 border-t-0 border-black text-xs">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="p-2">検査登録 （印紙代）</td>
                    <td className="p-2 text-right">¥{formatNumber(data.legalFees.inspection_registration_stamp)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">車庫証明 （印紙代）</td>
                    <td className="p-2 text-right">¥{formatNumber(data.legalFees.parking_certificate_stamp)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">下取車手続・処分 （印紙代）</td>
                    <td className="p-2 text-right">¥{formatNumber(data.legalFees.trade_in_stamp)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">リサイクル預託金</td>
                    <td className="p-2 text-right">¥{formatNumber(data.legalFees.recycling_deposit)}</td>
                  </tr>
                  <tr>
                    <td className="p-2">その他非課税費用</td>
                    <td className="p-2 text-right">¥{formatNumber(data.legalFees.other_nontaxable)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div>
              ...
              <table className="w-full border-2 border-t-0 border-black text-xs">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="p-2">検査登録手続代行</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.inspection_registration_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">車庫証明手続代行</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.parking_certificate_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">下取車手続・処分手続代行</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.trade_in_processing_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">下取車査定料</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.trade_in_assessment_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">リサイクル管理費用</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.recycling_management_fee)}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="p-2">納車費用</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.delivery_fee)}</td>
                  </tr>
                  <tr>
                    <td className="p-2">その他費用</td>
                    <td className="p-2 text-right">¥{formatNumber(data.processingFees.other_fees)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
                    ...
                    <tbody>
                      ...
                      <tr className="border-b border-black">
                        ...
                        <td className="p-3 text-right border-r border-black">
                          {data.loanCalculation ? `¥${formatNumber(data.loanCalculation.principal)}` : ""}
                        </td>
